...
import { SenderFieldSchema } from "@myorg/shared/form";
...
type FormValues = { sender: string };

// Отдельная форма поля: свой submit → PATCH только с этим полем.
export default function SenderForm({ value }: { value: ParcelDto }) {
  ...
  const {
    ...
    mode: "onChange",
    resolver: zodResolver(SenderFieldSchema),
    defaultValues: { sender: value.sender },
  });

  // Синхронизируем с сервером после ре-фетча.
  useEffect(() => {
    reset({ sender: value.sender }, { keepDirty: false });
  }, [value, reset]);

  const onSubmit = handleSubmit(async (data) => {
    try {
      await mutateAsync({ sender: data.sender });
      ...
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <StyledTextField
            {...field}
            fullWidth
            label={t("form.parcel.sender.label")}
            error={!!error}
            sx={{
              "& .MuiInputBase-root": {
                paddingRight: 1,
