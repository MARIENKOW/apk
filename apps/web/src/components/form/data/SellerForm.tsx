...
import { SellerFieldSchema } from "@myorg/shared/form";
...
type FormValues = { seller: string };

// Отдельная форма поля: свой submit → PATCH только с этим полем.
export default function SellerForm({ value }: { value: DataDto }) {
  ...
  const {
    ...
    mode: "onChange",
    resolver: zodResolver(SellerFieldSchema),
    defaultValues: { seller: value.seller },
  });

  // Синхронизируем с сервером после ре-фетча.
  useEffect(() => {
    reset({ seller: value.seller }, { keepDirty: false });
  }, [value, reset]);

  const onSubmit = handleSubmit(async (data) => {
    try {
      await mutateAsync({ seller: data.seller });
      ...
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <StyledTextField
            {...field}
            fullWidth
            label={t("form.data.seller.label")}
            error={!!error}
            sx={{
              "& .MuiInputBase-root": {
                paddingRight: 1,
