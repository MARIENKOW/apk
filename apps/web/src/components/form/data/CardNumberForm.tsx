...
import { CardNumberFieldSchema } from "@myorg/shared/form";
...
type FormValues = { cardNumber: string };

export default function CardNumberForm({ value }: { value: DataDto }) {
  ...
  const {
    ...
    mode: "onChange",
    resolver: zodResolver(CardNumberFieldSchema),
    defaultValues: { cardNumber: value.cardNumber },
  });

  useEffect(() => {
    reset({ cardNumber: value.cardNumber }, { keepDirty: false });
  }, [value, reset]);

  const onSubmit = handleSubmit(async (data) => {
    try {
      await mutateAsync({ cardNumber: data.cardNumber });
      ...
      <Controller
        ...
        render={({ field, fieldState: { error } }) => (
          <StyledTextField
            {...field}
            fullWidth
            label={t("form.data.cardNumber.label")}
            error={!!error}
